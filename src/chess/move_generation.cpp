...
MoveList<256> Board::legal_moves() const
{
  ...
  // {
  //   bitscanAll(bitboard_[piece_], src_bitscan);
  //   for (int i = 0; i < src_bitscan.len(); i++)
  //   {
  //     const Square src = u64ToSquare(src_bitscan[i]);
  ...
  //     bitscanAll(quiet_destinations, dest_bitscan);
  //     for (int k = 0; k < dest_bitscan.len(); k++)
  //     {
  //       const Square dest = u64ToSquare(dest_bitscan[k]); //all zeroes for some reason...
