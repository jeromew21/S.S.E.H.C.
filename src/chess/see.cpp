...
int Board::see(CMove mv) const
{
  ...
  u64 attack_set = attackers_to_(dest, White) | attackers_to_(dest, Black);
  ...
  {
    depth++;
    scores[depth] = getMaterialValue(piece_) - scores[depth - 1]; // capture
    ...
    attack_set |= move_maps::jumpingAttackers(dest, White, knights, 0, black_pawns) | move_maps::jumpingAttackers(dest, White, knights, 0, white_pawns);
    ...
    piece_ = least_valuable_piece_(attack_set, color, att_pos);
    used_attackers |= att_pos;    // add attacker to used set
    occ &= ~att_pos;              // remove attacking piece
    color = oppositeColor(color); // switch turn
  } while (!piece::is_empty(piece_));
